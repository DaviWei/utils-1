...
func lockKeyForName(name string) string {
	return fmt.Sprintf("github.com/soundtrackyourbrand/gaeutils/mutex.Mutex{Name:%v}", name)
}

func lockIdForName(c gaecontext.GAEContext, name string) *datastore.Key {
	return datastore.NewKey(c, "github.com/soundtrackyourbrand/gaeutils/mutex.Mutex{Name:%v}", name, 0, nil)
}
